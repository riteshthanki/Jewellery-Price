      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8 overflow-y-auto">
          ...
          <div className="space-y-4">
            <h4 className="text-lg font-bold text-slate-800 border-b border-slate-200 pb-2">Shop Identity</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
              ...
              <div className="flex flex-col space-y-2">
                <label className="text-sm font-bold text-slate-700 uppercase tracking-wide flex justify-between">
                  ...
                  <span className="text-xs font-normal text-slate-400 normal-case">For light backgrounds</span>
                </label>
                <div className="flex flex-col gap-3">
                  <div 
                    className="w-full aspect-video rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 flex items-center justify-center overflow-hidden cursor-pointer hover:border-amber-500 transition-colors relative"
                    onClick={() => fileInputRef.current?.click()}
                  >
                    {isUploading ? (
                       <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-600"></div>
                    ) : logoUrl ? (
                      <img src={logoUrl} alt="Light Logo" className="h-full w-auto object-contain p-2" />
                    ) : (
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 px-3 py-2 bg-slate-100 text-slate-700 font-medium rounded-lg hover:bg-slate-200 transition-colors text-xs"
                    >
                    ...
                    {logoUrl && (
                      <button 
                        type="button" 
                        onClick={() => setLogoUrl('')}
                        className="px-3 py-2 bg-red-50 text-red-500 hover:bg-red-100 rounded-lg transition-colors text-xs font-medium"
                      >
                        Remove
                      </button>
                    )}
              ...
              <div className="flex flex-col space-y-2">
                <label className="text-sm font-bold text-slate-700 uppercase tracking-wide flex justify-between">
                  ...
                  <span className="text-xs font-normal text-slate-400 normal-case">For dark backgrounds</span>
                </label>
                <div className="flex flex-col gap-3">
                  <div 
                    className="w-full aspect-video rounded-lg border-2 border-dashed border-slate-600 bg-slate-900 flex items-center justify-center overflow-hidden cursor-pointer hover:border-amber-500 transition-colors relative"
                    onClick={() => darkLogoInputRef.current?.click()}
                  >
                    {isDarkLogoUploading ? (
                       <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-600"></div>
                    ) : darkLogoUrl ? (
                      <img src={darkLogoUrl} alt="Dark Logo" className="h-full w-auto object-contain p-2" />
                    ) : (
          ...
          <div className="space-y-4">
            <h4 className="text-lg font-bold text-slate-800 border-b border-slate-200 pb-2">Theme & Appearance</h4>
            
            {/* Theme Selection Section */}
            <div className="flex flex-col space-y-3">
              <label className="text-sm font-bold text-slate-700 uppercase tracking-wide">Select Theme</label>
